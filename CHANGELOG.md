...
- Bumped `vitest` and `@vitest/coverage-v8` (dev) `^4.1.9` → `^4.1.11` — fixes a path traversal / arbitrary file read in `@vitest/mocker`'s redirect mock handling (GHSA-82fw-gwwq-j7x9)
